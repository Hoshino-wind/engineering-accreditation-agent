...
/** 安全解析错误响应体，失败时返回空对象 */
function readErrorBody(res: Response): Promise<AuthErrorBody> {
  return res
    .json()
    .then((body: unknown) =>
      body && typeof body === 'object' ? (body as AuthErrorBody) : {},
    )
    .catch(() => ({}));
}
  ...
  const onFinish = async (values: LoginFormValues) => {
    try {
      ...
      if (loginRes.status === 401) {
        const err = await readErrorBody(loginRes);
        form.setFields([
          {
            name: 'password',
      ...
      // Step 3: 跳转到专业选择页，让用户确认/选择当前专业
      // 带 auto=true：如果只有一个专业可自动绑定，减少首次登录点击
      const next = getNextParam();
      window.location.replace(
        `/select-major?next=${encodeURIComponent(next)}&auto=true`,
      );
    } catch (err) {
      messageApi.error(
